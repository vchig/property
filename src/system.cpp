...
int property::system::propertyCount() const { return static_cast< int >( properties_.size() ); }

std::string property::system::propertyName( int i ) const
{
    if( propertyCount() <= i ) return std::string();
...
{
    if( propertyCount() <= i ) return true;
    auto pos = properties_.begin();
    std::advance( pos, i );
...
{
    if( propertyCount() <= i ) return boost::any();
    auto pos = properties_.begin();
    std::advance( pos, i );
...
{
    if( properties_.find( name ) != properties_.end() ) return properties_[name]->get();
    return boost::any();
}
...
void property::system::setProperty( const std::string &name, const boost::any &value )
{
    if( properties_.find( name ) != properties_.end() ) {
        ...
        if( !prop->is_read_only() ) prop->set( value );
    }
}

void property::system::setProperty( int i, const boost::any &value )
{
    if( propertyCount() <= i ) return;
    auto pos = properties_.begin();
    std::advance( pos, i );
